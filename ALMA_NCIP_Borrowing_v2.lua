...
--Modified again by:  Bill Jones III (SUNY Geneseo) and Tim Jackson (SUNYL SLSS)
...
--Change ILLiad field for grabbing the External Identifier sent from Alma
Settings.ILLiad_field_for_External_Identifier = GetSetting("ILLiad_field_to_get_external_identifier");
...
local lender = GetFieldValue("Transaction", "LendingLibrary");
	if lender == nil then
		lender = "";
	end
	if string.find(lender, "&") ~= nil then
		lender = string.gsub(lender, "&", "and");
	end

local numberofpieces = GetFieldValue("Transaction", "Pieces");
	if numberofpieces == nil then
		numberofpieces = "";
	end
	if string.find(numberofpieces, "&") ~= nil then
		numberofpieces = string.gsub(numberofpieces, "&", "and");
	end

local barcode = GetFieldValue("Transaction", Settings.ILLiad_field_to_get_barcode);
	if barcode == "" then
		barcode = GetFieldValue("Transaction","TransactionNumber");
	end
	if string.find(barcode, "&") ~= nil then
		barcode = string.gsub(barcode, "&", "and");
	end
	...
	if sublibraries ~= nil then
		for line in sublibraries:lines() do
			if string.find(line, pickup_location_full) ~= nil then
				pickup_location = string.sub(line, line:len() - 3);
				break;
...
local user = GetFieldValue("Transaction", "Username");
if Settings.Use_Prefixes then
	local t = GetFieldValue("Transaction", Settings.ILLiad_field_to_get_barcode);
	if t == nil then
		t = GetFieldValue("Transaction","TransactionNumber");
	end
	if GetFieldValue("Transaction", "LibraryUseOnly") and GetFieldValue("Transaction", "RenewalsAllowed") then
	    tn = Settings.Prefix_for_LibraryUseOnly_and_RenewablesAllowed .. t;
	end
	...
	tn = Settings.acceptItem_Transaction_Prefix .. GetFieldValue("Transaction", Settings.ILLiad_field_to_get_barcode);
	if tn == "" then
		tn = GetFieldValue("Transaction","TransactionNumber");
	end
end
